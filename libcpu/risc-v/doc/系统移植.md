...
进入异常，中断前，硬件将自动设置mstatus.MPIE等于mstatus.MIE，保存MIE的状态到MPIE中，然后硬件自动将mstatus.MIE字段设置为0，表示关闭全局中断。



当之前mret执行退出异常，中断后，硬件自动将mstatus.MIE赋值为mstatus.MPIE,相当于恢复了中断配置
